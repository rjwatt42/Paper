...
doublePlot(shapes,"k",resultK,Llabel,resultNull,Plabel,resultS,Slabel)
...
doublePlot(c("Unique","Duplicated"),"",kRes,Llabel,nRes,Plabel)
...
doublePlot(siblings,"siblings",kRes,Llabel,nRes,Plabel)
...
doublePlot(studies,"studies",kRes,Llabel,nRes,Plabel)
...
doublePlot(years,"year",kRes,Llabel,nRes,Plabel)
...
for (i in 1:(length(journals))) {
  ...
  wall<-w*(1-an$best$Nullmax)+0.05*an$best$Nullmax
  wRes<-c(wRes,wall)
  # fdr
  fdr<-0.05*an$best$Nullmax/wall
  fdrRes<-c(fdrRes,fdr)
}

use<-order(nmean)
doublePlot(journals[use],"",matrix(kRes[use],1),Llabel,matrix(nRes[use],1),Plabel)
doublePlot(journals[use],"",matrix(wRes[use],1),"w",matrix(fdrRes[use],1),"fdr")
...
doublePlot(factors,"Impact",kRes,Llabel,nRes,Plabel)
...
doublePlot(statistics,"Test",kRes,Llabel,nRes,Plabel)
...
doublePlot(cases,"Tails",kRes,Llabel,nRes,Plabel)
...
for (i in 1:length(alphas)) {
  ...
  use<-my_data_sim$p<alpha
  metaData<-list(result=list(rIV=my_data_sim$r_s[use],nval=my_data_sim$n[use],df1=my_data_sim$df1[use]))
  an<-runMetaAnalysis(metaAnal,metaData)
  nRes_sim<-cbind(nRes_sim,an$best$Nullmax)
  kRes_sim<-cbind(kRes_sim,an$best$Kmax)
  sRes_sim<-cbind(sRes_sim,an$best$Smax)
  showAnalysis(an,paste0("sim:alpha=",alphas[i]))
}
alpha<-0.05

doublePlot(alphas,"alpha",rbind(kRes,kRes_sim),Llabel,rbind(nRes,nRes_sim),Plabel,xlog=TRUE)
...
doublePlot(siblings,"",rbind(tkRes,rkRes),Llabel,rbind(tnRes,rnRes),Plabel,legendLabels=c("t","r"))
